    ...
    <Layout title="Car leasing Cyprus" description={metaDescription}>
      <section className="">
        <div className="flex flex-row max-sm:block">
          <h1
            className="font-bold text-highlight1 w-[400px] left-11 top-32 md:absolute text-5xl lg:w-[600px]
                    max-md:text-4xl max-md:flex max-md:p-5 max-md:w-full"
          >
            Discover the advantages of renting long-term.
          </h1>
          <img
        ...
        <div className="w-full bg-bglight flex flex-col my-4 lg:hidden">
          <div className="flex justify-between px-2 gap-4">
            <div className="flex flex-col justify-end -translate-y-4">
              <span
                onClick={() =>
                  setIndex(index - 1 < 0 ? infoBoxes.length - 1 : index - 1)
                }
                className="left arrow"
              ></span>
            </div>
            <div className="grid grid-col-1 mt-4">{infoBoxes[index]}</div>
            <div className="flex flex-col justify-end -translate-y-4 -translate-x-6">
              <span
                onClick={() => setIndex((index + 1) % infoBoxes.length)}
                className="right arrow"
              ></span>
            </div>
          </div>
          <div className="flex justify-center gap-2 mb-2">{dots}</div>
        </div>
        ...
        <div className="p-4 ">
          <h2 className="text-highlight1 text-3xl md:px-10">
            Monthly car rental
          </h2>
          <p className="md:px-10">
            Our long term contract is perfect for anyone who needs a car for an
            extended period of time. You can choose from our wide range of
            vehicles, from economy to luxury, and drive around the beautiful
            island of Cyprus with ease.
          </p>
        </div>
