...
export default function LongTermEnquiry(props) {
  ...
  React.useEffect(() => {
    if (props.longTermStep !== 2 || !props.longTermCar)
  ...
  async function handleSubmit(event) {
    ...
    if (
      ...
      event.target.querySelector("#email-confirm").value
    ) {
      ...
      await fetch("api/mail", {
        ...
        .catch((e) => {
          ...
          props.setIsLoading(false);
          alert(
            "An Error has occurred. Please check the email address provided."
          );
        });
    }
  }
  ...
  return (
    <Layout title="Book your Car" description={metaDescription}>
          ...
          <div className="flex flex-col-reverse gap-x-16 gap-y-8 lg:grid lg:grid-cols-5">
            <div className="rounded-lg bg-white p-8 shadow-lg lg:col-span-2 lg:p-12">
              <form action="" className="space-y-4">
                ...
                <div className="flex flex-row m-0 align-middle">
                  <input
                    checked={enquiryDetails.delivery}
                    onChange={() =>
                      setEnquiryDetails((prev) => {
                        return { ...prev, delivery: !prev.delivery };
                      })
                    }
                  ...
                  />
                  <label
                    className="not-sr-only pt-1 text-lg px-2"
                    htmlFor="delivery"
                  >
                    Delivery service
                  </label>
                ...
                <p className="text-sm m-0">
                  Have the car delivered to your desired location, for a small
                  fee. An agent will reach out to confirm.
                </p>
                <hr />
                <div className="flex flex-row gap-3">
                  <div>
                    <label className="not-sr-only" htmlFor="pickupDate">
                      Pick-up Date{" "}
                    </label>
                    <Flatpickr
                  ...
                  <div>
                    <label className="not-sr-only" htmlFor="period">
                      Period{" "}
                    </label>
                    <input
            ...
            <div className="flex flex-col bg-white shadow-lg rounded-lg lg:col-span-3">
              <div className="bg-dark text-white px-3 flex flex-row">
                <h3>
                  {props.longTermCar.name}
                  <small className="pl-3 text-sm">
                    {props.longTermCar.year}
                  </small>
                </h3>
              </div>
              <div className="flex flex-col gap-4 md:flex-row">
                <Carousel
                  photos={props.longTermCar.photos}
                  thumb={props.longTermCar.thumb}
                />
                <div>
                  <div className="p-3 md:grid md:grid-cols-2 md:grid-rows-2 flex justify-around">
                    <span className="m-2 flex flex-col oject-center text-center w-15">
                      <img
                        className="h-6 object-scale-down pl-2"
                        src="icons\gas-pump-solid.svg"
                        alt="fuel icon"
                      />
                      <span className="text-xs m-0">Fuel</span>
                      <span className="text-md font-semibold m-0">
                        {props.longTermCar.fuel}
                      </span>
                    </span>
                    <span className="m-2 flex flex-col oject-center text-center w-15">
                      <img
                        className="h-6 object-scale-down"
                        src="icons\gas.png"
                        alt="gas icon"
                      />
                      <p className="text-xs m-0">Consumption</p>
                      <p className="text-md font-semibold m-0">
                        {props.longTermCar.fuelConsumption}{" "}
                        <small className="text-sm font-thin">lt/100km</small>
                      </p>
                    </span>
                    <span className="m-2 flex flex-col oject-center text-center w-15">
                      <img
                      ...
                      <p className="text-md font-semibold m-0">
                        {props.longTermCar.number_doors}
                      </p>
                    </span>
                    <span className="m-2 flex flex-col oject-center text-center w-15">
                      <img
                      ...
                      <p className="text-md font-semibold m-0">
                        {props.longTermCar.transmission}
                      </p>
                    </span>
                  </div>
                  <div>
                    <h3 className="font-black text-center">
                      {utils.toCurrency(props.longTermCar.monthlyPrice, true)}
                      <small className="text-sm font-normal"> / month</small>
                    </h3>
                    <p className="text-sm px-4">
                      Price includes VAT, unlimited km, Maintenance and repair,
                      Insurance, Replacement in case of damage, Road tax,
                      Roadside assistance{" "}
                    </p>
                  </div>
                </div>
      ...
      <section className="bg-gray-100">
        <h3 className="text-center max-w-screen-xl px-4">
          Fill in your contact details below, send us an enquiry and we will
          contact you back.
        </h3>
            ...
            <div className="rounded-lg bg-white p-8 shadow-lg lg:col-span-5 lg:p-12">
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    <label className="sr-only" htmlFor="email-confirm">
                      Email confirmation
                    </label>
                    <input
                      className="w-full rounded-lg border-gray-200 p-3 text-sm"
