...
function formatEnquiryEmail(enquiryDetails) {
  return `An enquiry for a long term rental has been made. Details follow.
...
function formatAllureContactEmail(enquiryDetails) {
  return `An enquiry has been made. Details follow.
_________________________________________________________________
...
export default async function handler(req, res) {
  ...
  if (JSON.parse(req.body).type === "booking") {
    const enquiryMailOptions = {
      ...
      subject: `Leasing Enquiry - ${JSON.parse(req.body).info.car} - ${
        JSON.parse(req.body).info.name
      }`,
